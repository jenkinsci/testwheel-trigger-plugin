...
public class TestWheelTrigger extends Builder implements SimpleBuildStep {
	...
	public TestWheelTrigger(String apiKey, String prjctKey) {
		...
		this.prjctKey = Secret.fromString(prjctKey);
	}
	...
	public void perform(Run<?, ?> run, FilePath workspace, EnvVars env, Launcher launcher, TaskListener listener) {
		try (CloseableHttpClient client = createHttpClientWithProxy()) {
				...
				if (runId != null && !runId.isEmpty()) {
					...
					while (true) {
						...
						if ("SUCCESS".equalsIgnoreCase(secondJsonResponse.getString(STATUS))) {
							...
							client.execute(reportRequest, response -> {
								try (InputStream reportStream = response.getEntity().getContent();
										OutputStream fos = reportFilePath.write()) {
									...
									listener.getLogger()
											.println("Report download failed: " + e.getMessage());
								}
								return null; 
							});
			...
			listener.getLogger().println("Unable to Connect to URL: " + e1.getMessage());
			run.setResult(Result.FAILURE);
		}
	}
	...
	private CloseableHttpClient createHttpClientWithProxy()
		...
		var clientBuilder = HttpClients.custom().setConnectionManager(connManager);
		...
		if (proxyConfig != null) {
			...
			if (proxyConfig.getUserName() != null && proxyConfig.getSecretPassword() != null) {
				...
				UsernamePasswordCredentials credentials = new UsernamePasswordCredentials(proxyConfig.getUserName(),
			...
			clientBuilder.setProxy(proxy).setDefaultCredentialsProvider(credentialsProvider);
		}
